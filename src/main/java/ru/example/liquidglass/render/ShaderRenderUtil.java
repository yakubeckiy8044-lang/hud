...
public final class ShaderRenderUtil {
    ...
    public static void drawGlassPanel(DrawContext context, int x, int y, int width, int height, float radius) {
        ...
        float[] vertices = { x, y, 0, 0, x, y + height, 0, 1, x + width, y + height, 1, 1, x + width, y, 1, 0 };
        RenderSystem.enableBlend();
        RenderSystem.defaultBlendFunc();
        RenderSystem.disableCull();
        GL20.glUseProgram(program);
        GL20.glUniform2f(GL20.glGetUniformLocation(program, "ScreenSize"), screenWidth, screenHeight);
        GL20.glUniform4f(GL20.glGetUniformLocation(program, "Panel"), x, screenHeight - y - height, width, height);
        GL20.glUniform1f(GL20.glGetUniformLocation(program, "Radius"), radius);
        GL30.glBindVertexArray(vao);
        GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, vbo);
